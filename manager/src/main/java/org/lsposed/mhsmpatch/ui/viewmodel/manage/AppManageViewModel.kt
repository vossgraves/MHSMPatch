...
class AppManageViewModel : ViewModel() {
    ...
    init {
        viewModelScope.launch {
            snapshotFlow { MHSMPackageManager.appList }
    ...
    private fun loadData(silent: Boolean = false) {
        ...
        val currentList = MHSMPackageManager.appList.mapNotNull { appInfo ->
            runCatching {
                appInfo.app.metaData?.getString("mhsmpatch")?.let {
            ...
            withContext(Dispatchers.IO) {
                ...
                for (apk in apkPaths) {
                    ZipFile(apk).use { zip ->
                        ...
                        if (entry == null) entry = zip.getEntry("assets/mhsmpatch/origin_apk.bin")
