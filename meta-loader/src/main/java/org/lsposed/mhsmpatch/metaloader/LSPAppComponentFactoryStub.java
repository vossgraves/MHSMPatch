package org.lsposed.mhsmpatch.metaloader;
...
public class LSPAppComponentFactoryStub extends AppComponentFactory {
    ...
    private static final String TAG = "MHSMPatch-MetaLoader";
    ...
    static {
        ...
        if (appZygote) {
            Log.i(TAG, "Skip loading libmhsmpatch.so for appZygote");
        } else {
            bootstrap();
        }
    }
    ...
    private static void bootstrap() {
        try {
            ...
            if (useManager) {
                ...
                soPath = manager.sourceDir + "!/assets/mhsmpatch/so/" + libName + "/libmhsmpatch.so";
            } else {
                ...
                java.net.URL url = cl.getResource("assets/mhsmpatch/so/" + libName + "/libmhsmpatch.so");
                if (url == null) {
                    throw new RuntimeException("Should not happen: libmhsmpatch.so not found in assets");
                }
