...
public class SigBypass {
    ...
    private static final String TAG = "OQPatch-SigBypass";
        ...
        if (hasSignature) {
            ...
            if (replacement == null && !signatures.containsKey(packageName)) {
                try {
                    ...
                    String encoded = null;
                    if (metaData != null) encoded = metaData.getString("oqpatch");
                    if (encoded != null) {
    ...
    static void doSigBypass(Context context, int sigBypassLevel) throws IOException {
        ...
        if (sigBypassLevel >= Constants.SIGBYPASS_LV_PM_OPENAT) {
            ...
            try (ZipFile sourceFile = new ZipFile(context.getPackageResourcePath())) {
                cacheApkPath = context.getCacheDir() + "/oqpatch/origin/" + sourceFile.getEntry(ORIGINAL_APK_ASSET_PATH).getCrc() + ".apk";
            }
