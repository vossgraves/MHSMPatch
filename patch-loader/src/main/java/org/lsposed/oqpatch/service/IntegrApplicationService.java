package org.lsposed.oqpatch.service;
...
import org.lsposed.oqpatch.util.ModuleLoader;
...
public class IntegrApplicationService extends ILSPApplicationService.Stub {
    ...
    public IntegrApplicationService(Context context) {
        try {
            String[] assetsList = context.getAssets().list("oqpatch/modules");
            ...
            for (var name : assetsList) {
                ...
                String modulePath = context.getCacheDir() + "/oqpatch/" + packageName + "/";
                String cacheApkPath;
                ...
                if (!Files.exists(Paths.get(cacheApkPath))) {
                    ...
                    try (var is = context.getAssets().open("oqpatch/modules/" + name)) {
