...
import static org.lsposed.mhsmpatch.share.Constants.CONFIG_ASSET_PATH;
import static org.lsposed.mhsmpatch.share.Constants.EMBEDDED_MODULES_ASSET_PATH;
import static org.lsposed.mhsmpatch.share.Constants.LOADER_DEX_ASSET_PATH;
import static org.lsposed.mhsmpatch.share.Constants.ORIGINAL_APK_ASSET_PATH;
import static org.lsposed.mhsmpatch.share.Constants.PROXY_APP_COMPONENT_FACTORY;
...
public class MHSMPatch {
    ...
    public MHSMPatch(Logger logger, String... args) {
    ...
    public static void main(String... args) throws IOException {
        MHSMPatch mhsmpatch = new MHSMPatch(new JavaLogger(), args);
        if (mhsmpatch.help) {
            mhsmpatch.jCommander.usage();
            return;
        }
        try {
            mhsmpatch.doCommandLine();
        } catch (PatchError e) {
    ...
    public void doCommandLine() throws PatchError, IOException {
        for (var apk : apkPaths) {
            ...
            File outputFile = new File(outputDir, String.format(
                    Locale.getDefault(), "%s-%d-mhsmpatched.apk",
    ...
    public void patch(File srcApkFile, File outputFile) throws PatchError, IOException {
        ...
        try (ZFile dstZFile = ZFile.openReadWrite(outputFile, Z_FILE_OPTIONS);
            ...
            // save mhsmpatch config to asset..
            ...
            if (isInjectProvider){
                try (var is = getClass().getClassLoader().getResourceAsStream("assets/mtprovider.dex")) {
                    dstZFile.add("assets/mhsmpatch/mtprovider.dex", is);
                } catch (Throwable e) {
                    throw new PatchError("Error when adding dex", e);
                }
            }
            ...
            if (!useManager) {
                ...
                // do not put libmhsmpatch.so into apk!lib because x86 native bridge causes crash
                for (String arch : ARCHES) {
                    String entryName = "assets/mhsmpatch/so/" + arch + "/libmhsmpatch.so";
    ...
    private byte[] modifyManifestFile(InputStream is, String metadata, int minSdkVersion, String originPackage, String newPackage, String originalSignature) throws IOException {
        ...
        modules.forEach(module -> {
            ...
            property.addMetaData(new ModificationProperty.MetaData("xposeddescription", "MHSMPatch Embed Module"));
            property.addMetaData(new ModificationProperty.MetaData("xposedminversion", "93"));
        });

        property.addMetaData(new ModificationProperty.MetaData("mhsmpatch", metadata));
