...
import static org.lsposed.oqpatch.share.Constants.EMBEDDED_MODULES_ASSET_PATH;
import static org.lsposed.oqpatch.share.Constants.LOADER_DEX_ASSET_PATH;
import static org.lsposed.oqpatch.share.Constants.ORIGINAL_APK_ASSET_PATH;
import static org.lsposed.oqpatch.share.Constants.PROXY_APP_COMPONENT_FACTORY;
...
public class OQPatch {
    ...
    public OQPatch(Logger logger, String... args) {
    ...
    public static void main(String... args) throws IOException {
        OQPatch oqpatch = new OQPatch(new JavaLogger(), args);
        if (oqpatch.help) {
            oqpatch.jCommander.usage();
            return;
        }
        try {
            oqpatch.doCommandLine();
        } catch (PatchError e) {
    ...
    public void doCommandLine() throws PatchError, IOException {
        for (var apk : apkPaths) {
            ...
            File outputFile = new File(outputDir, String.format(
                    Locale.getDefault(), "%s-%d-oqpatched.apk",
        ...
        try (ZFile dstZFile = ZFile.openReadWrite(outputFile, Z_FILE_OPTIONS);
            ...
            if (isInjectProvider){
                try (var is = getClass().getClassLoader().getResourceAsStream("assets/mtprovider.dex")) {
                    dstZFile.add("assets/oqpatch/mtprovider.dex", is);
                } catch (Throwable e) {
                    throw new PatchError("Error when adding dex", e);
                }
            }
            ...
            if (!useManager) {
                ...
                // do not put liboqpatch.so into apk!lib because x86 native bridge causes crash
                for (String arch : ARCHES) {
                    String entryName = "assets/oqpatch/so/" + arch + "/liboqpatch.so";
    ...
    private byte[] modifyManifestFile(InputStream is, String metadata, int minSdkVersion, String originPackage, String newPackage, String originalSignature) throws IOException {
        ...
        modules.forEach(module -> {
            ...
            property.addMetaData(new ModificationProperty.MetaData("xposeddescription", "OQPatch Embed Module"));
            property.addMetaData(new ModificationProperty.MetaData("xposedminversion", "93"));
        });

        property.addMetaData(new ModificationProperty.MetaData("oqpatch", metadata));
