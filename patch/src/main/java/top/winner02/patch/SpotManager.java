package top.winner02.spotmanager.patch;

import static top.winner02.spotmanager.share.Constants.CONFIG_ASSET_PATH;
import static top.winner02.spotmanager.share.Constants.EMBEDDED_MODULES_ASSET_PATH;
import static top.winner02.spotmanager.share.Constants.LOADER_DEX_ASSET_PATH;
import static top.winner02.spotmanager.share.Constants.ORIGINAL_APK_ASSET_PATH;
import static top.winner02.spotmanager.share.Constants.PROXY_APP_COMPONENT_FACTORY;
...
import top.winner02.spotmanager.share.LSPConfig;
import top.winner02.spotmanager.share.PatchConfig;
import top.winner02.spotmanager.patch.util.ApkSignatureHelper;
import top.winner02.spotmanager.patch.util.JavaLogger;
import top.winner02.spotmanager.patch.util.Logger;
import top.winner02.spotmanager.patch.util.ManifestParser;
...
public class SpotManager {
    ...
    private static final String SPOTMANAGER_KEYSTORE_PASSWORD_ENC = "a2hpbm9s";
    private static final String SPOTMANAGER_KEY_ALIAS_ENC = "MT8jag==";
    ...
    @Parameter(names = {"-npa", "--spotmanager-keystore"}, description = "Use built-in SpotManager keystore")
    private boolean useSpotmanagerKeystore = false;
    ...
    public SpotManager(Logger logger, String... args) {
        ...
        if (keystoreArgs != null && (useSpotmanagerKeystore || useFpaKeystore)) {
            logger.e("Cannot use -k with -npa or -fpa\n");
            help = true;
        }
        if (useSpotmanagerKeystore && useFpaKeystore) {
    ...
    public static void main(String... args) throws IOException {
        SpotManager spotmanager = new SpotManager(new JavaLogger(), args);
        if (spotmanager.help) {
            spotmanager.jCommander.usage();
            return;
        }
        try {
            spotmanager.doCommandLine();
        } catch (PatchError e) {
    ...
    public void doCommandLine() throws PatchError, IOException {
        for (var apk : apkPaths) {
            ...
            File outputFile = new File(outputDir, String.format(
                    Locale.getDefault(), "%s-%d-spotmanagered.apk",
    ...
    public void patch(File srcApkFile, File outputFile) throws PatchError, IOException {
        ...
        try (ZFile dstZFile = ZFile.openReadWrite(outputFile, Z_FILE_OPTIONS);
            ...
            try {
                ...
                if (useSpotmanagerKeystore || (!useFpaKeystore && keystoreArgs == null)) {
                    logger.i("Register apk signer with built-in SpotManager keystore...");
                    registerBuiltinSigner(keyStore, dstZFile, "assets/spotmanager.key", SPOTMANAGER_KEYSTORE_PASSWORD_ENC, SPOTMANAGER_KEY_ALIAS_ENC);
            ...
            // save spotmanager config to asset..
            ...
            if (isInjectProvider){
                try (var is = getClass().getClassLoader().getResourceAsStream("assets/mtprovider.dex")) {
                    dstZFile.add("assets/spotmanager/mtprovider.dex", is);
                } catch (Throwable e) {
                    throw new PatchError("Error when adding dex", e);
                }
            }
            ...
            if (!useManager) {
                ...
                // do not put libspotmanager.so into apk!lib because x86 native bridge causes crash
                for (String arch : ARCHES) {
                    String entryName = "assets/spotmanager/so/" + arch + "/libspotmanager.so";
    ...
    private byte[] modifyManifestFile(InputStream is, String metadata, int minSdkVersion, String originPackage, String newPackage, String originalSignature) throws IOException {
        ...
        modules.forEach(module -> {
            ...
            property.addMetaData(new ModificationProperty.MetaData("xposeddescription", "SpotManager Embed Module"));
            property.addMetaData(new ModificationProperty.MetaData("xposedminversion", "93"));
        });

        property.addMetaData(new ModificationProperty.MetaData("spotmanager", metadata));
